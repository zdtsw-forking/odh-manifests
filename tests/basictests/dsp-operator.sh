...
EOF
    ...
    os::cmd::try_until_not_text "curl -s -k -H 'Authorization: Bearer ${SA_TOKEN}' https://${ROUTE}/apis/v1beta1/runs/${RUN_ID} | jq .run.status" "null" $odhdefaulttimeout $odhdefaultinterval
}
...
EOF
    ...
    os::cmd::try_until_not_text "curl -s -k -H 'Authorization: Bearer ${SA_TOKEN}' https://${ROUTE}/apis/v1beta1/runs/${RUN_ID_EXPT} | jq .run.status " "null" $odhdefaulttimeout $odhdefaultinterval
}
...
EOF
    ...
    os::cmd::try_until_not_text "curl -s -k -H 'Authorization: Bearer ${SA_TOKEN}' https://${ROUTE}/apis/v1beta1/jobs/${JOB_ID} | jq .status " "null" $odhdefaulttimeout $odhdefaultinterval
}
